...
{		
	...
	/**
 	 ...
 	 * @var String Instancia de conexión
 	 */ 
    ...
    public static function getInstance($dataBase = null)
    {
		...
		$dbData = unserialize(BDCONNECTIONS);		

    	$dbParams = $dbData[$dataBase];

		
    	
    	//$dbParams = BDCONNECTIONS[$dataBase];
		
    	if(!$dbParams)
    	{
    		PwFunciones::setLogError(33);
    		return false;    		
    	}
    	...
    	switch ($dataBase) {
    		case 1:    			
				...
				if(!extension_loaded($dbType))
				{
					...
					PwFunciones::setLogError(50, $dbType);
					return null;
				}				
			...
			case 2:    			
				...
				$dbUsername = $dbParams["username"];				
				$dbPassword = $dbParams["password"];				
				$dbCadena = "oci:dbname=".$dbParams["host"].';charset=UTF8';
			break;
			...
			case 4:    			
				...
				$dbUsername = $dbParams["username"];				
				$dbPassword = $dbParams["password"];				
				//$dbCadena = "mysql:host=".$dbParams["host"].";Database=".$dbParams["db"];
				$dbCadena = "sqlsrv:Server=".$dbParams["host"].";Database=".$dbParams["db"];				
			break;
    	}
    	...
    	if (!isset(self::$instance[$dataBase]))
		{
			...
			try 
			{
				self::$instance[$dataBase]->connection = new \PDO($dbCadena,$dbUsername,$dbPassword, $options);
				self::$instance[$dataBase]->connection->setAttribute(\PDO::ATTR_ERRMODE, \PDO::ERRMODE_EXCEPTION); 	

			}
			catch (PDOException $e)
			{
				PwFunciones::setLogError(30, $e->getMessage());
				return null;
			}		
		}
	 ...
	 */
	/*public static function getPdoConnection($database = 2, $dbUsernameAux = false, $dbPasswordAux = false, $dbCadenaAux = false, $dbCharsetAux = false, $dbHostAux = false, $dbNameAux = false)
	{
...
/**
	 ...
	 */
	/*public function getConnection_($database= 2, $dbUsernameAux = false, $dbPasswordAux = false, $dbHostAux = false, $dbNameAux = false,  $dbCharsetAux = false)
	{
